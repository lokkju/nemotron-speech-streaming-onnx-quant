...
- **quantize.py** — Applies int8 dynamic quantization (`onnxruntime.quantization.quantize_dynamic`) and int4 weight-only quantization (`MatMul4BitsQuantizer`) to encoder and decoder_joint models
...
- int4 `MatMul4BitsQuantizer` may not support all ops; fall back to GPTQ or RTN block-wise quantization if needed.
